...
class RecordCacheTest < Test::Unit::TestCase
  ...
  test 'find_by_id with space' do
    dog = Dog.create(:name => 'Frankie')
    assert Pet.find_by_id("#{dog.id} ")
  end

  test 'create field lookup functions' do
  ...
  test 'return cached values without accessing the database' do
  ...
  test 'return multiple cached values without accessing the database' do
  ...
  test 'create raw find methods' do
  ...
  test 'cache indexes using scope' do
  ...
  test 'invalidate indexes on save' do
